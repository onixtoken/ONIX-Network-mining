...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string, username?: string, isRegister?: boolean, retries = 2) => {
    try {
      ...
      if (!isJson) {
        const text = await res.text();
        console.error("Server returned non-JSON response. Status:", res.status, "Body:", text);
        ...
        // If it's a 404, the route might be missing
        if (res.status === 404) {
          throw new Error("Login service temporarily unavailable (404). Please try again in a moment.");
        }
        
        throw new Error(`Server error (${res.status}): Invalid response format. Check console for details.`);
      }
